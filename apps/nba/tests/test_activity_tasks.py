...
class TestBroadcastNextActivityEvent:
    def test_no_pending_events_does_nothing(self):
        ...
        with patch("activity.tasks.get_channel_layer") as mock_get_layer:
    ...
    def test_broadcasts_oldest_queued_event(self):
        ...
        with patch("activity.tasks.get_channel_layer", return_value=mock_layer):
            with patch("activity.tasks.async_to_sync") as mock_async_to_sync:
    ...
    def test_marks_event_as_broadcast(self):
        ...
        mock_layer = MagicMock()
        with patch("activity.tasks.get_channel_layer", return_value=mock_layer):
            with patch("activity.tasks.async_to_sync") as mock_sync:
