...
class TestCreateCommentView:
    def test_unauthenticated_redirected(self):
        ...
        game = GameFactory()
        response = c.post(
            f"/game/{game.id_hash}/comments/create/",
            {"body": "Great game!"},
        )
    ...
    def test_game_not_found_returns_404(self, auth_client):
        c, user = auth_client
        response = c.post(
            "/game/nonexistent/comments/create/",
            {"body": "test"},
        )
...
class TestCreateReplyView:
    def test_unauthenticated_redirected(self):
        ...
        response = c.post(
            f"/game/{game.id_hash}/comments/{parent.pk}/reply/",
            {"body": "Great reply!"},
        )
        assert response.status_code in (301, 302)
    ...
    def test_game_not_found_returns_404(self, auth_client):
        ...
        parent_id = 999
        response = c.post(
            f"/game/nonexistent/comments/{parent_id}/reply/",
            {"body": "test"},
        )
        assert response.status_code == 404
    ...
    def test_parent_comment_not_found_returns_404(self, auth_client):
        ...
        game = GameFactory()
        response = c.post(
            f"/game/{game.id_hash}/comments/999999/reply/",
            {"body": "test"},
        )
        assert response.status_code == 404
    ...
    def test_parent_from_different_game_returns_404(self, auth_client):
        ...
        response = c.post(
            f"/game/{game2.id_hash}/comments/{parent.pk}/reply/",
            {"body": "test"},
        )
        assert response.status_code == 404
