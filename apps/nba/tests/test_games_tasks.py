...
class TestFetchTeams:
    @patch("games.tasks.sync_teams", return_value=30)
    def test_success_returns_count(self, mock_sync):
        result = fetch_teams()
        assert result == {"synced": 30}
        mock_sync.assert_called_once()

    @patch("games.tasks.sync_teams", side_effect=Exception("API down"))
...
class TestFetchLiveScores:
    @patch("games.tasks.sync_live_scores", return_value=5)
    def test_success_returns_count(self, mock_sync):
        result = fetch_live_scores()
        assert result == {"updated": 5}
        mock_sync.assert_called_once()

    @patch("games.tasks.sync_live_scores", side_effect=Exception("error"))
    def test_retries_on_failure(self, mock_sync):
