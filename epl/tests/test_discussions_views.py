"""Tests for discussions/views.py — CommentListView, CreateCommentView, CreateReplyView, DeleteCommentView."""
...
class TestCommentListView:
    def test_returns_200(self):
        match = MatchFactory()
        c = Client()
        resp = c.get(f"/epl/match/{match.slug}/comments/")
    ...
    def test_invalid_offset_defaults_to_zero(self):
        match = MatchFactory()
        c = Client()
        resp = c.get(f"/epl/match/{match.slug}/comments/?offset=abc")
        assert resp.status_code == 200
    ...
    def test_404_for_invalid_slug(self):
        c = Client()
        resp = c.get("/epl/match/no-such-match/comments/")
...
class TestCreateCommentView:
    def test_unauthenticated_redirected(self):
        ...
        c = Client()
        resp = c.post(
            f"/epl/match/{match.slug}/comments/create/",
            {"body": "Great match!"},
        )
    ...
    def test_match_not_found_returns_404(self, auth_client):
        c, user = auth_client
        resp = c.post(
            "/epl/match/nonexistent/comments/create/",
            {"body": "test"},
        )
        assert resp.status_code == 404
...
class TestCreateReplyView:
    def test_unauthenticated_redirected(self):
        ...
        c = Client()
        resp = c.post(
            f"/epl/match/{match.slug}/comments/{parent.pk}/reply/",
            {"body": "Reply!"},
        )
        assert resp.status_code in (301, 302)
    ...
    def test_parent_not_found_returns_404(self, auth_client):
        ...
        match = MatchFactory()
        resp = c.post(
            f"/epl/match/{match.slug}/comments/999999/reply/",
            {"body": "test"},
        )
        assert resp.status_code == 404
    ...
    def test_parent_from_different_match_returns_404(self, auth_client):
        ...
        resp = c.post(
            f"/epl/match/{match2.slug}/comments/{parent.pk}/reply/",
            {"body": "test"},
        )
        assert resp.status_code == 404
...
class TestDeleteCommentView:
    def test_unauthenticated_redirected(self):
        ...
        c = Client()
        resp = c.post(
            f"/epl/match/{match.slug}/comments/{comment.pk}/delete/",
        )
        assert resp.status_code in (301, 302)
    ...
    def test_comment_not_found_returns_404(self, auth_client):
        ...
        match = MatchFactory()
        resp = c.post(
            f"/epl/match/{match.slug}/comments/999999/delete/",
        )
        assert resp.status_code == 404
