"""
Futures market settlement for EPL.
...
from epl.betting.models import FuturesBet, FuturesMarket, FuturesOutcome
...
def settle_futures_market(market_pk: int, winner_team_pk: int) -> dict:
    ...
    # Settle bets outside the market lock to avoid long transactions
    ...
    for bet in pending_bets:
        with transaction.atomic():
            if bet.outcome_id == winning_outcome.pk:
                payout = (bet.stake * bet.odds_at_placement).quantize(Decimal("0.01"))
                ...
                log_transaction(
                    balance,
                    payout,
                    BalanceTransaction.Type.FUTURES_WIN,
    ...
    for bet in pending_bets:
        with transaction.atomic():
            ...
            log_transaction(
                balance,
                bet.stake,
                BalanceTransaction.Type.FUTURES_VOID,
