"""
Celery tasks for NFL betting: odds generation, settlement, and futures.
"""
...
from nfl.betting.futures_odds_engine import generate_futures_odds
from nfl.betting.models import Odds
from nfl.betting.odds_engine import generate_all_upcoming_odds
...
def generate_odds(self):
    """Generate algorithmic House odds for all upcoming NFL games."""
    ...
    try:
        ...
        if created > 0:
            ...
            queue_activity_event(
                "odds_update",
                f"Fresh NFL odds generated ({created} new lines)",
                url="/nfl/",
                icon="chart-line-up",
            )
...
def settle_pending_bets(self):
    """Find FINAL/FINAL_OT games with unsettled bets and settle them."""
    try:
        from nfl.betting.models import BetSlip, ParlayLeg
        from nfl.betting.settlement import settle_game_bets
        ...
        game_pks = set(
            BetSlip.objects.filter(
                status=BetStatus.PENDING, game__status__in=final_statuses
            )
...
def update_futures_odds(self):
    """Regenerate futures odds for all open NFL markets from current standings."""
    ...
    try:
        ...
        for market in markets:
            results = generate_futures_odds(season, market.market_type, market.division)
            if not results:
                continue
