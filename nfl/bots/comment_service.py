"""Bot comment generation service for NFL — thin delegation to centralized pipeline."""

from nfl.bots.adapter import FOOTBALL_KEYWORDS, nfl_adapter
...
def _homer_team_mentioned(profile, text):
    return homer_team_mentioned(nfl_adapter, profile, text)


def _is_bot_relevant(profile, game):
    return nfl_adapter.is_bot_relevant(profile, game)
...
def _build_user_prompt(
    ...
    match_ctx = nfl_adapter.build_match_context(game)
...
def generate_bot_comment(
    ...
    return generate_comment(
        nfl_adapter, bot_user, game, trigger_type, bet_slip, parent_comment
    )


def select_reply_bot(game, target_comment):
    return _core_select_reply(nfl_adapter, game, target_comment)
