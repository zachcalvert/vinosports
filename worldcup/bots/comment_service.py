...
def generate_bot_comment(
    ...
    """Generate and post an LLM-powered comment for a bot user on a World Cup match."""
...
# Cache: country_code → (name_lower, short_name_lower, tla_lower, country_code_lower) | None
_homer_team_cache: dict[str, tuple[str, str, str, str] | None] = {}


def _homer_team_mentioned(profile, text):
    """Return True if the bot's national team is referenced in the comment text."""
    if not profile.worldcup_country_code:
        return False

    code_key = profile.worldcup_country_code
    if code_key not in _homer_team_cache:
        from worldcup.matches.models import Team

        team = Team.objects.filter(country_code=code_key).first()
        if not team:
            _homer_team_cache[code_key] = None
        else:
            _homer_team_cache[code_key] = (
                team.name.lower(),
    ...
    cached = _homer_team_cache.get(code_key)
    if not cached:
        return False

    name_lower, short_lower, tla_lower, cc_lower = cached
    ...
    for abbr in (tla_lower, cc_lower):
        if abbr and re.search(rf"\b{re.escape(abbr)}\b", text_lower):
            return True
...
def _build_user_prompt(
    ...
    if match.group:
        lines.append(f"Group {match.group.letter}")
    if match.venue:
        lines.append(f"Venue: {match.venue}, {match.city}")
