"""Tests for worldcup bet settlement.
...
class TestSettleMatchBets:
    ...
    def test_home_win_bet_wins_when_home_wins_then_loses_on_penalties(self):
        """2-1 at 90 min → HOME_WIN bet wins, ignore that this is a knockout."""
        match = FinishedMatchFactory(
            home_score=2,
            away_score=1,
            # hypothetical: match was 2-1 at 90 but extra time was replayed
            # (edge case — in practice this won't happen, but settlement is
            # always on home_score/away_score)
        )
        home_bet = BetSlipFactory(
            match=match,
            selection=BetSlip.Selection.HOME_WIN,
    ...
    def test_does_not_raise_for_missing_match(self):
        ...
        settle_match_bets(999999)  # non-existent pk
    ...
    def test_does_not_resettle_already_settled_bets(self):
        ...
        # Pending count was 0, already-won bet unchanged
