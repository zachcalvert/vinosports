...
class TestRunBotStrategies:
    ...
    @patch("nba.bots.tasks.roll_action", return_value=False)
    def test_skips_when_roll_fails(self, mock_roll):
        BotProfileFactory(is_active=True)
        result = run_bot_strategies()
        assert result["dispatched"] == 0

    @patch("nba.bots.tasks.execute_bot_strategy.apply_async")
    @patch("nba.bots.tasks.roll_action", return_value=True)
    @patch("nba.bots.tasks.get_active_window")
    ...
    @patch("nba.bots.tasks.execute_bot_strategy.apply_async")
    @patch("nba.bots.tasks.roll_action", return_value=True)
    @patch("nba.bots.tasks.get_active_window", return_value=None)
