"""Bot betting strategies for UCL.

UCL uses decimal 1X2 odds (home win / draw / away win) only — same as EPL
and World Cup. Strategy calibration reflects European club football: draws are
common (~25%), home advantage is meaningful, and knockout ties add two-leg drama.
...
class DrawSpecialistStrategy(BaseStrategy):
    """Always bets the draw — draws occur ~25% of the time in European football."""
...
class HomerStrategy(BaseStrategy):
    """Bets on the bot's favourite club every time they play.

    Uses ``profile.ucl_team_tla`` to identify the team via
    ``ucl_matches.Team.tla``.
    """

    def _resolve_team_id(self):
        if not self.profile.ucl_team_tla:
            return None
        if not hasattr(self, "_team_id_cache"):
            from ucl.matches.models import Team

            team = Team.objects.filter(tla=self.profile.ucl_team_tla).first()
...
class AntiHomerStrategy(BaseStrategy):
    """Bets AGAINST the bot's favourite club every time they play.

    Uses ``profile.ucl_team_tla`` to find the team, then backs the
