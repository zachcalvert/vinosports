"""UCL bot tasks — strategy execution and comment generation."""
...
from ucl.bots.comment_service import generate_bot_comment, select_reply_bot
...
def run_bot_strategies():
    ...
    active_bots = BotProfile.objects.filter(is_active=True, active_in_ucl=True)
...
def execute_bot_strategy(bot_profile_pk):
    """Execute a single bot's betting strategy for UCL matches."""
    try:
    ...
    if not profile.is_active or not profile.active_in_ucl:
        return
...
def generate_bot_comment_task(
    ...
    """Generate and post a bot comment for a UCL match."""
    try:
        bot_user = User.objects.get(pk=bot_user_pk)
        match = Match.objects.select_related("home_team", "away_team", "stage").get(
            pk=match_pk
        )
...
def generate_prematch_comments(bot_user_ids=None):
    """Dispatch pre-match hype comments for upcoming UCL matches."""
    now = timezone.now()
...
def generate_postmatch_comments(bot_user_ids=None):
    """Dispatch post-match reaction comments for finished UCL matches."""
    now = timezone.now()
...
def maybe_reply_to_human_comment(match_pk, comment_pk):
    """Possibly trigger a bot reply to a human comment on a UCL match."""
    try:
...
def generate_featured_parlays():
    """Build themed UCL parlays for the featured section."""
    logger.info("Generating UCL featured parlays")
