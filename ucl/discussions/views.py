    ...
    def get(self, request, match_slug):
        ...
        if offset > 0:
            html = render_to_string(
                "ucl_discussions/partials/comment_page.html",
                context,
                request=request,
            )
        else:
            html = render_to_string(
                "ucl_discussions/partials/comment_list.html",
                context,
                request=request,
            )
...
class CreateCommentView(LoginRequiredMixin, View):
    def post(self, request, match_slug):
        ...
        if not form.is_valid():
            html = render_to_string(
                "ucl_discussions/partials/comment_form.html",
        ...
        if not request.user.is_bot:
            try:
                from ucl.bots.tasks import maybe_reply_to_human_comment
        ...
        html = render_to_string(
            "ucl_discussions/partials/comment_single.html",
            {"comment": comment, "match": match, "depth": 0},
            request=request,
        )
        html += render_to_string(
            "ucl_discussions/partials/comment_count_oob.html",
