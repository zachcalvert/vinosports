...
_UCL_PARLAY_SESSION_KEY = "ucl_parlay_slip"
...
def _ucl_get_slip(request):
    return list(request.session.get(_UCL_PARLAY_SESSION_KEY, []))


def _ucl_save_slip(request, slip):
    request.session[_UCL_PARLAY_SESSION_KEY] = slip
    request.session.modified = True


def _ucl_build_slip_context(request):
    """Build parlay slip template context."""
...
class OddsBoardView(TemplateView):
    template_name = "ucl_betting/odds_board.html"
...
class OddsBoardPartialView(OddsBoardView):
    template_name = "ucl_betting/partials/odds_board_body.html"
...
class QuickBetFormView(LoginRequiredMixin, View):
    ...
    def get(self, request, match_slug):
        ...
        return render(
            request,
            "ucl_betting/partials/quick_bet_form.html",
            {
                "match": match,
...
class PlaceBetView(LoginRequiredMixin, View):
    ...
    def _error_template(self, container_id):
        if container_id:
            return "ucl_betting/partials/quick_bet_form.html"
        return "ucl_betting/partials/bet_form.html"
    ...
    def post(self, request, match_slug):
        ...
        return render(
            request,
            "ucl_betting/partials/bet_confirmation.html",
            {
                "bet": bet,
                "match": match,
...
class AddToParlayView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if not match_id or selection not in dict(BetSlip.Selection.choices):
            return render(
                request,
                "ucl_betting/partials/parlay_slip.html",
                {
                    **_ucl_build_slip_context(request),
                    "parlay_error": "Invalid selection.",
                },
            )

        slip = _ucl_get_slip(request)
...
class RemoveFromParlayView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        slip = [e for e in _ucl_get_slip(request) if e["match_id"] != match_id]
        _ucl_save_slip(request, slip)
...
class PlaceParlayView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        def _error(msg):
            ctx = _ucl_build_slip_context(request)
            ctx["parlay_error"] = msg
            return render(request, "ucl_betting/partials/parlay_slip.html", ctx)
        ...
        return render(
            request,
            "ucl_betting/partials/parlay_confirmation.html",
            {
                "parlay": parlay,
                "leg_data": leg_data,
                "combined_odds": combined_odds,
...
class FuturesView(TemplateView):
    """List all open UCL futures markets."""

    template_name = "ucl_betting/futures/futures_list.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx = super().get_context_data(**kwargs)
        season = getattr(django_settings, "UCL_CURRENT_SEASON", "2025")
        ...
        for market in markets:
            if market.market_type == FuturesMarket.MarketType.WINNER:
                outcomes = (
                    FuturesOutcome.objects.filter(market=market)
                    .select_related("team")
                    .order_by("odds")[:10]
                )
                ...
                top_8 = {"market": market, "outcomes": outcomes}
...
class FuturesMarketDetailView(TemplateView):
    ...
    template_name = "ucl_betting/futures/futures_detail.html"
...
class FuturesBetFormView(LoginRequiredMixin, View):
    ...
    def get(self, request, id_hash):
        ...
        return render(
            request,
            "ucl_betting/futures/partials/_bet_form.html",
...
class PlaceFuturesBetView(LoginRequiredMixin, View):
    ...
    def post(self, request, id_hash):
        ...
        return render(
            request,
            "ucl_betting/futures/partials/_bet_confirmation.html",
            {
                "bet": bet,
                "outcome": outcome,
                "potential_payout": potential_payout,
