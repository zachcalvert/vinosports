"""Tests for ucl/betting/views.py — odds board, bet placement, quick bet form."""
...
class TestPlaceBetView:
    def test_unauthenticated_redirected(self, client):
        match = MatchFactory()
        resp = client.post(
            reverse("ucl_betting:place_bet", args=[match.slug]),
    ...
    def test_match_not_found(self, auth_client):
        c, user = auth_client
        resp = c.post(
            reverse("ucl_betting:place_bet", args=["nonexistent-match"]),
            {"selection": "HOME_WIN", "stake": "50.00"},
        )
...
class TestQuickBetFormView:
    def test_requires_login(self, client):
        match = MatchFactory()
        resp = client.get(reverse("ucl_betting:quick_bet_form", args=[match.slug]))
        assert resp.status_code == 302
    ...
    def test_renders_form(self, auth_client):
        ...
        OddsFactory(match=match)
        resp = c.get(
            reverse("ucl_betting:quick_bet_form", args=[match.slug])
    ...
    def test_match_not_found(self, auth_client):
        c, user = auth_client
        resp = c.get(reverse("ucl_betting:quick_bet_form", args=["nonexistent-match"]))
        assert resp.status_code == 404
